...
public class MovieBase extends ResponseBusinessObjectDAO {
	...
	protected int updateChildrenId(long[] ids, int index, int parentIndex) {
        for (final MovieLink item : movieLink) {
            index = item.updateId(ids, index, parentIndex);
        }

        for (final MovieCategory item : movieCategory) {
            index = item.updateId(ids, index, parentIndex);
        }
    ...
    public void addChildsIntoDatabase(final ArrayList<ContentProviderOperation> batch, final int previousResult) {
		for (final MovieLink item : movieLink) {
            item.addIntoDatabase(batch, MovieLink.CONTENT_URI, MovieLink.FIELD__PARENT_ID, previousResult);
        }
		for (final MovieCategory item : movieCategory) {
            item.addIntoDatabase(batch, MovieCategory.CONTENT_URI, MovieCategory.FIELD__PARENT_ID, previousResult);
        }
    }
    ...
    public void addChildsIntoDatabase(final ArrayList<ContentProviderOperation> batch) {
		for (final MovieLink item : movieLink) {
            item._parentId = _id;
            item.addIntoDatabase(batch, MovieLink.CONTENT_URI);
        }
		for (final MovieCategory item : movieCategory) {
            item._parentId = _id;
            item.addIntoDatabase(batch, MovieCategory.CONTENT_URI);
        }
    }
