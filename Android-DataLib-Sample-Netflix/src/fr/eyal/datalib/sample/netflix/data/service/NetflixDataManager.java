...
public class NetflixDataManager extends DataManager {
    ...
    @Override
    public ResponseBusinessObject getBusinessObjectFromCacheByUrl(final int type, final String url) {

        switch (type) {
            case NetflixService.WEBSERVICE_NEWRELEASES:
            	return new NewReleases(url);
            case NetflixService.WEBSERVICE_MOVIEIMAGE:
            	return new MovieImage(url);
     ...
     * @throws UnsupportedEncodingException
     */
	public synchronized int getMovieImage(final int policy, final OnDataListener datacacheListener, final String url, final int options) throws UnsupportedEncodingException {
		//we prepare the parameters
		final ParameterMap params = new ParameterMap();
        ...
        int requestId = launchRequest(mServiceHelper, policy, datacacheListener, params, options, __url, NetflixService.WEBSERVICE_MOVIEIMAGE, NetflixService.class);
