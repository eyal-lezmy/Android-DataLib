package fr.eyal.datalib.sample.netflix.data.model.people;
...
public class PeopleBase extends ResponseBusinessObjectDAO {
	...
	//list of childs
	public ArrayList<PeopleLink> peopleLink;

    public PeopleBase() {
        super();
    }

    public PeopleBase(final long id) {
        super(id);
    }

    public PeopleBase(final String url) {
        super(url);
    }
     ...
     */
    public static final String CONTENT_PATH = "people";
     ...
     */
    public static String DATABASE_TABLE_NAME = "people";
    ...
    protected void fillObjectFromCursor(final Cursor cursor) {
		// we initialize the childs tabs
        	peopleLink = new ArrayList<PeopleLink>();
    ...
    public void deleteChildsFromDatabase(final ArrayList<ContentProviderOperation> batch) {
        ...
        whereClause = PeopleLink.FIELD__PARENT_ID + "=?"; // peoplelink_id=?
        batch.add(ContentProviderOperation.newDelete(PeopleLink.CONTENT_URI)
    ...
    protected void fillChildrenFromDatabase() {
        ...
        columns = new String[PeopleLink.DATABASE_TABLE_FIELDS_NAMES.length];
        for (int i = 0; i < PeopleLink.DATABASE_TABLE_FIELDS_NAMES.length; i++) {
            columns[i] = PeopleLink.DATABASE_TABLE_FIELDS_NAMES[i];
        }

        where = PeopleLink.FIELD__PARENT_ID + "=?";
        args = new String[1];
        args[0] = _id + "";

        // we check the existence of the entry inside the database
        cursor = mResolver.query(PeopleLink.CONTENT_URI,
        ...
        if (cursor.moveToFirst()) {
            ...
            do {
                ...
                final PeopleLink itemPeopleLink = new PeopleLink();
                itemPeopleLink.fillObjectFromCursor(cursor);
                peopleLink.add(itemPeopleLink); // we add a new object to the list
	...
	/**
     * This function build an array of {@link PeopleBase} thanks to a Cursor
     ...
     *  	   Ex: Cast to ArrayList<{@link People}> if you want it as {@link People}
     */
    public static ArrayList<?> buildArrayFromCursor(final Cursor c, final boolean join) {

        final ArrayList<PeopleBase> result = new ArrayList<PeopleBase>();

        if (c.moveToFirst()) {
            do {
                // we create and fill the item
                final PeopleBase newObject = new PeopleBase();
	...
	public static final Parcelable.Creator<PeopleBase> CREATOR = new Parcelable.Creator<PeopleBase>() {
	    @Override
	    public PeopleBase createFromParcel(final Parcel in) {
	        return new PeopleBase(in);
	    }
	
	    @Override
	    public PeopleBase[] newArray(final int size) {
	        return new PeopleBase[size];
	    }
	};
	...
	public void writeToParcel(final Parcel dest, final int flags) {
		...
		//list of childs
		dest.writeParcelableArray(peopleLink.toArray(new PeopleLink[peopleLink.size()]), 0);
	}

	public PeopleBase(final Parcel in) {
		// ResponseBusinessObject DAO
		_ttl = in.readLong();
		_url = in.readString();
		...
		//list of children
		Parcelable[] items; 
		items = in.readParcelableArray(PeopleLink.class.getClassLoader());
		peopleLink = new ArrayList<PeopleLink>();
		for (final Parcelable parcelable : items) {
		    peopleLink.add((PeopleLink) parcelable);
		}
