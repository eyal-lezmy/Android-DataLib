...
public class ItemTopGenreBase extends BusinessObjectDAO {
    ...
    public ItemTopGenreBase() {
        super();
    }

    public ItemTopGenreBase(final long id) {
        super(id);
    }
     ...
     */
    public static final String CONTENT_PATH = "itemtopgenre";
     ...
     */
    public static String DATABASE_TABLE_NAME = "itemtopgenre";
    ...
    public static String CREATE_TABLE = "CREATE TABLE " + DATABASE_TABLE_NAME + " (" 
            ...
            + TopGenre.DATABASE_TABLE_NAME + "(" + TopGenre.DATABASE_TABLE_FIELDS_NAMES[0] + ") ON DELETE CASCADE" + ", "
    ...
    protected void fillFromDatabaseById(final long id) {
        ...
        final Cursor cursor = mResolver.query(CONTENT_URI, // ItemTopGenre
                columns, // id
                where, // id=?
                args, // id
	...
	/**
     * This function build an array of {@link ItemTopGenreBase} thanks to a Cursor
     ...
     *  	   Ex: Cast to ArrayList<{@link ItemTopGenre}> if you want it as {@link ItemTopGenre}
     */
    public static ArrayList<?> buildArrayFromCursor(final Cursor c, final boolean join) {

        final ArrayList<ItemTopGenreBase> result = new ArrayList<ItemTopGenreBase>();

        if (c.moveToFirst()) {
            do {
                // we create and fill the item
                final ItemTopGenreBase newObject = new ItemTopGenreBase();
	...
	public static final Parcelable.Creator<ItemTopGenreBase> CREATOR = new Parcelable.Creator<ItemTopGenreBase>() {
	    @Override
	    public ItemTopGenreBase createFromParcel(final Parcel in) {
	        return new ItemTopGenreBase(in);
	    }
	
	    @Override
	    public ItemTopGenreBase[] newArray(final int size) {
	        return new ItemTopGenreBase[size];
	    }
	};
	...
	public ItemTopGenreBase(final Parcel in) {
		// Business Object DAO
		_id = in.readLong();
		_updatedAt = Calendar.getInstance();
