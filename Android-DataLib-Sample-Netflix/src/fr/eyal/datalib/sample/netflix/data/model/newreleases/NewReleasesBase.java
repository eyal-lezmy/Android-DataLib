...
public class NewReleasesBase extends ResponseBusinessObjectDAO {
	...
	protected int updateChildrenId(long[] ids, int index, int parentIndex) {
        for (final Item itemElement : item) {
            index = itemElement.updateId(ids, index, parentIndex);
        }
    ...
    public void addChildsIntoDatabase(final ArrayList<ContentProviderOperation> batch, final int previousResult) {
		for (final Item itemElement : item) {
            itemElement.addIntoDatabase(batch, Item.CONTENT_URI, Item.FIELD__PARENT_ID, previousResult);
        }
    }
    ...
    public void addChildsIntoDatabase(final ArrayList<ContentProviderOperation> batch) {
		for (final Item itemElement : item) {
            itemElement._parentId = _id;
            itemElement.addIntoDatabase(batch, Item.CONTENT_URI);
        }
    }
    ...
    public void deleteChildsFromDatabase(final ArrayList<ContentProviderOperation> batch) {
        ...
        for (final Item itemElement : item) {
            itemElement.deleteChildsFromDatabase(batch);
        }
        ...
        whereClause = Item.FIELD__PARENT_ID + "=?"; // item_id=?
        batch.add(ContentProviderOperation.newDelete(Item.CONTENT_URI)
    ...
    protected void fillChildrenFromDatabase() {
        ...
        columns = new String[Item.DATABASE_TABLE_FIELDS_NAMES.length];
        for (int i = 0; i < Item.DATABASE_TABLE_FIELDS_NAMES.length; i++) {
            columns[i] = Item.DATABASE_TABLE_FIELDS_NAMES[i];
        }

        where = Item.FIELD__PARENT_ID + "=?";
        args = new String[1];
        args[0] = _id + "";

        // we check the existence of the entry inside the database
        cursor = mResolver.query(Item.CONTENT_URI,
        ...
        if (cursor.moveToFirst()) {
            ...
            do {
                ...
                final Item itemItem = new Item();
                itemItem.fillObjectFromCursor(cursor);
                item.add(itemItem); // we add a new object to the list
	...
	public void writeToParcel(final Parcel dest, final int flags) {
		...
		//list of childs
		dest.writeParcelableArray(item.toArray(new Item[item.size()]), 0);
	}
	...
	public NewReleasesBase(final Parcel in) {
		...
		//list of children
		Parcelable[] items; 
		items = in.readParcelableArray(Item.class.getClassLoader());
		item = new ArrayList<Item>();
		for (final Parcelable parcelable : items) {
		    item.add((Item) parcelable);
		}
