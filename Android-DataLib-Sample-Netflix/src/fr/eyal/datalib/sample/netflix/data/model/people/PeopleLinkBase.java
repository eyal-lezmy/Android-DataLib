...
public class PeopleLinkBase extends BusinessObjectDAO {
    ...
    public PeopleLinkBase() {
        super();
    }

    public PeopleLinkBase(final long id) {
        super(id);
    }
     ...
     */
    public static final String CONTENT_PATH = "peoplelink";
     ...
     */
    public static String DATABASE_TABLE_NAME = "peoplelink";
    ...
    public static String CREATE_TABLE = "CREATE TABLE " + DATABASE_TABLE_NAME + " (" 
            ...
            + People.DATABASE_TABLE_NAME + "(" + People.DATABASE_TABLE_FIELDS_NAMES[0] + ") ON DELETE CASCADE" + ", "
    ...
    protected void fillFromDatabaseById(final long id) {
        ...
        final Cursor cursor = mResolver.query(CONTENT_URI, // PeopleLink
                columns, // id
                where, // id=?
                args, // id
	...
	/**
     * This function build an array of {@link PeopleLinkBase} thanks to a Cursor
     ...
     *  	   Ex: Cast to ArrayList<{@link PeopleLink}> if you want it as {@link PeopleLink}
     */
    public static ArrayList<?> buildArrayFromCursor(final Cursor c, final boolean join) {

        final ArrayList<PeopleLinkBase> result = new ArrayList<PeopleLinkBase>();

        if (c.moveToFirst()) {
            do {
                // we create and fill the item
                final PeopleLinkBase newObject = new PeopleLinkBase();
	...
	public static final Parcelable.Creator<PeopleLinkBase> CREATOR = new Parcelable.Creator<PeopleLinkBase>() {
	    @Override
	    public PeopleLinkBase createFromParcel(final Parcel in) {
	        return new PeopleLinkBase(in);
	    }
	
	    @Override
	    public PeopleLinkBase[] newArray(final int size) {
	        return new PeopleLinkBase[size];
	    }
	};
	...
	public PeopleLinkBase(final Parcel in) {
		// Business Object DAO
		_id = in.readLong();
		_updatedAt = Calendar.getInstance();
